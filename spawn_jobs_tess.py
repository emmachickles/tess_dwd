...
ticid_c = [int(f.split('_')[12][3:]) for f in fnames_c if f.split('.')[-1] == 'png']
ticid_c = np.array(ticid_c)
inter, comm1, comm2 = np.intersect1d(ticid, ticid_c, return_indices=True)
ticid = np.delete(ticid, comm1) 

p = np.array([ np.ones(ticid.shape)*sector, np.ones(ticid.shape)*cam,
               np.ones(ticid.shape)*ccd, ticid ]).T.astype('int')

if __name__ == "__main__":
    N_p=8 # >> uzay has 64 cores, 4 GPUs => 16 CPUs per GPU
    multiprocessing.set_start_method('spawn')
    pool = Pool(processes=N_p)
    result = pool.map(run_process, p) # ticid, sig, snr, wid, period, period_min, q, phi0, dur, epo
    np.savetxt(gpu_dir+'GPU-{}-{}-{}.result'.format(sector, cam, ccd), np.array(result),
               fmt='%i,%10.5f,%10.5f,%i,%10.5f,%10.5f,%10.5f,%10.5f,%10.5f,%10.5f')
